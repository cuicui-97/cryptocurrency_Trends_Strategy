...
class BinanceDataFetcher(BaseDataFetcher):
    ...
    def __init__(self, symbol: str = config.SYMBOL,
                 max_messages: int | None = config.MAX_MESSAGES):
        super().__init__(symbol, max_messages)
        self._ws = None
        # 成交和订单簿分别用独立 logger，写到不同文件
        self.trade_logger = make_logger("data.trade", config.TRADE_LOG_FILE)
        self.book_logger = make_logger("data.orderbook", config.ORDER_BOOK_LOG_FILE)
    ...
    async def _handle_trade(self, data: dict) -> None:
        """
        aggTrade 字段：
          m=True  → 主动卖出（taker sell）
          m=False → 主动买入（taker buy）
        """
        trade = {
            'instId': self.instId,
            'tradeId': str(data.get('a')),
            'px': data.get('p'),
            'sz': data.get('q'),
    ...
    def _handle_order_book(self, data: dict) -> None:
        # 合约用 'b'/'a'，现货用 'bids'/'asks'，兼容两种格式
        order_book = {
            "bids": data.get("b") or data.get("bids", []),
            "asks": data.get("a") or data.get("asks", []),
        }
        self.order_book_deque.append(order_book)

        # 每50次更新记录一次，避免日志量过大
        if len(self.order_book_deque) % 50 == 0:
            bids = order_book['bids']
            asks = order_book['asks']
            if bids and asks:
                self.book_logger.info(
                    f"买一={bids[0][0]} 卖一={asks[0][0]} "
                    f"买一量={bids[0][1]} 卖一量={asks[0][1]}"
                )
